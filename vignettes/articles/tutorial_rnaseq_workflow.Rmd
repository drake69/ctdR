...
expression matrix** plus a **`design`** and a **`contrast`**, and
recomputes the differential signal internally. The chemical→gene "index"
is built by `ctdR` from CTD targets. We reuse the `design` matrix built
for limma; here `design` carries the whole layout while `contrast = 2`
selects *which* coefficient to test — the `groupDex` effect (Dex vs DMSO).
